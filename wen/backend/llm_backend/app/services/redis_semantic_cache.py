...
class RedisSemanticCache:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    return result["embeddings"][0]  # 一次只处理一条文本，返回第一个向量，也是唯一一个
    ...
    def _get_vector_key(self, message: str) -> str:
        """生成向量存储的键名"""
        # message.encode() - 字符串编码为字节
        # hashlib.md5(...) - 计算 MD5 哈希
        ...
        while True:
            try:
                ...
                if len(all_keys) > self.max_cache_size:
                    ...
                    # 按最后访问时间排序
                    # key=lambda x: x[1]：指定排序的规则——使用每个元素的第二个值作为排序依据

...
class SearchService:
    ...
    async def _call_with_tool(self, query: str) -> Dict:
        ...
        try:
            ...
            # 获取原始错误信息
            raw_error = str(e)
            # 转义所有可能被 loguru 解析的字符
            safe_error = raw_error.replace("{", "{{").replace("}", "}}")
            logger.error(f"Error in _call_with_tool: {safe_error}", exc_info=True)
            raise
    ...
    async def generate_stream(
        ...
        # 在 generate_stream 方法中，找到错误处理部分
        except Exception as e:
            # 修改这一行：
            # logger.error(f"Error in generate_stream: {str(e)}", exc_info=True)
            # 改为：
            error_msg = str(e).replace("{", "{{").replace("}", "}}")
            logger.error(f"Error in generate_stream: {error_msg}", exc_info=True)